...
#include <qt/glitchgui.h>

#include <qt/glitchunits.h>
...
#include <qt/glitchversionchecker.h>
    ...
    platformStyle(_platformStyle)
{
    ...
    glitchVersionChecker = new GlitchVersionChecker(this);
...
void BitcoinGUI::createActions()
{
    ...
    sendCoinsAction->setStatusTip(tr("Send coins to a Glitch address"));
    ...
    receiveCoinsAction->setStatusTip(tr("Request payments (generates QR codes and glitch: URIs)"));
    ...
    QRCTokenAction = new QAction(platformStyle->MultiStatesIcon(":/icons/grctoken"), tr("&QRC Tokens"), this);
    ...
    signMessageAction->setStatusTip(tr("Sign messages with your Glitch addresses to prove you own them"));
    verifyMessageAction = new QAction(platformStyle->MenuColorIcon(":/icons/verify"), tr("&Verify message..."), this);
    verifyMessageAction->setStatusTip(tr("Verify messages to ensure they were signed with specified Glitch addresses"));
    ...
    openAction->setStatusTip(tr("Open a glitch: URI or payment request"));//GLITCH_LINE

    showHelpMessageAction = new QAction(platformStyle->MenuColorIcon(":/icons/info"), tr("&Command-line options"), this);
    showHelpMessageAction->setMenuRole(QAction::NoRole);
    showHelpMessageAction->setStatusTip(tr("Show the %1 help message to get a list with possible Glitch command-line options").arg(tr(PACKAGE_NAME)));
...
void BitcoinGUI::setClientModel(ClientModel *_clientModel)
{
    ...
    {
        // Check for updates
        if(_clientModel->getOptionsModel()->getCheckForUpdates() && glitchVersionChecker->newVersionAvailable())
        {
            QString link = QString("<a href=%1>%2</a>").arg(GLITCH_RELEASES, GLITCH_RELEASES);
            QString message(tr("New version of Glitch wallet is available on the Glitch source code repository: <br /> %1. <br />It is recommended to download it and update this application").arg(link));
...
void BitcoinGUI::updateNetworkState()
{
    ...
    if (m_node.getNetworkActive()) {
        tooltip = tr("%n active connection(s) to Glitch network", "", count) + QString(".<br>") + tr("Click to disable network activity.");
    } else {
...
{
    QString strTitle = tr("Glitch"); // default title
