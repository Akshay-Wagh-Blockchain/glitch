...
###   glitchcore.org and glitch.org and compares them.
...
WORKINGDIR="/tmp/glitch_verify_binaries"
TMPFILE="hashes.tmp"

SIGNATUREFILENAME="SHA256SUMS.asc"
RCSUBDIR="test"
HOST1="https://glitchcore.org"
HOST2="https://glitch.org"
BASEDIR="/bin/"
VERSIONPREFIX="glitch-core-"
RCVERSIONSTRING="rc"
...
if [ -n "$1" ]; then
   #let's also check if the version number includes the prefix 'glitch-',
...
if [ $? -ne 0 ]; then
   echo "glitch.org failed to provide signature file, but glitchcore.org did?"
   echo "wget output:"
   echo "$WGETOUT"|sed 's/^/\t/g'
...
if [ "$SIGFILEDIFFS" != "" ]; then
   echo "glitch.org and glitchcore.org signature files were not equal?"
