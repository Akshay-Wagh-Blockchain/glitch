...
bool CBlockTreeDB::ReadStakeIndex(unsigned int height, uint160& address){
    ...
    while (pcursor->Valid()) {
        boost::this_thread::interruption_point();
        std::pair<char, CHeightTxIndexKey> key;
        pcursor->GetKey(key); //note: it's apparently ok if this returns an error https://github.com/glitch/glitch/issues/7890
    ...
    while (pcursor->Valid()) {
        ...
        if (pcursor->GetKey(key) && key.first == DB_BLOCK_INDEX) {
            ...
            if (pcursor->GetValue(diskindex)) {
                ...
                pindexNew->hashStateRoot  = diskindex.hashStateRoot; // glitch
                pindexNew->hashUTXORoot   = diskindex.hashUTXORoot; // glitch
                pindexNew->nStakeModifier = diskindex.nStakeModifier;
                pindexNew->prevoutStake   = diskindex.prevoutStake;
                pindexNew->vchBlockSig    = diskindex.vchBlockSig; // glitch
