...
bool ToHash160(const std::string& strGlitchAddress, std::string& strHash160)
{
    CTxDestination glitchAddress = DecodeDestination(strGlitchAddress);
    if(!IsValidDestination(glitchAddress))
        return false;
    const CKeyID * keyid = boost::get<CKeyID>(&glitchAddress);
    if(keyid){
...
bool ToGlitchAddress(const std::string& strHash160, std::string& strGlitchAddress)
{
    uint160 key(ParseHex(strHash160.c_str()));
    CKeyID keyid(key);
    CTxDestination glitchAddress = keyid;
    if(IsValidDestination(glitchAddress)){
        strGlitchAddress = EncodeDestination(glitchAddress);
        return true;
    }
    return false;
}
...
bool Token::execEvents(int64_t fromBlock, int64_t toBlock, int func, std::vector<TokenEvent> &tokenEvents)
{
    ...
    for(int i = 0; i < list.size(); i++)
    {
        ...
        for(int i = 0; i < listLog.size(); i++)
        {
            ...
            ToGlitchAddress(tokenEvent.sender, tokenEvent.sender);
            tokenEvent.receiver = topicsList[2].toString().toStdString().substr(24);
            ToGlitchAddress(tokenEvent.receiver, tokenEvent.receiver);
