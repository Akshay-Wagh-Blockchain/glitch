...
#define paternVersion "glitch-([0-9]+\\.)?([0-9]+\\.)?([0-9]+)-"

GlitchVersionChecker::GlitchVersionChecker(QObject *parent) : QObject(parent)
{
    currentVersion = Version(CLIENT_VERSION_MAJOR, CLIENT_VERSION_MINOR, CLIENT_VERSION_REVISION);
}

GlitchVersionChecker::~GlitchVersionChecker()
{

}

bool GlitchVersionChecker::newVersionAvailable()
{
    Version maxReleaseVersion = getMaxReleaseVersion();
    return maxReleaseVersion > currentVersion;
}

QList<Version> GlitchVersionChecker::getVersions()
{
    QNetworkAccessManager manager;
    QNetworkReply *response = manager.get(QNetworkRequest(QUrl(GLITCH_RELEASES)));
    QEventLoop event;
...
Version GlitchVersionChecker::getMaxReleaseVersion()
{
