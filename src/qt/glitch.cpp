...
{
    return QCoreApplication::translate("glitch-core", psz).toStdString();
}
...
static void initTranslations(QTranslator &qtTranslatorBase, QTranslator &qtTranslator, QTranslator &translatorBase, QTranslator &translator)
{
    ...
    // Load e.g. glitch_de.qm (shortcut "de" needs to be defined in glitch.grc)
    if (translatorBase.load(lang, ":/translations/"))
        QApplication::installTranslator(&translatorBase);

    // Load e.g. glitch_de_DE.qm (shortcut "de_DE" needs to be defined in glitch.grc)
...
void BitcoinApplication::initializeResult(bool success)
{
    ...
    if(success)
    {
        ...
        // glitch: URIs or payment requests:
...
{
    QMessageBox::critical(0, "Runaway exception", BitcoinGUI::tr("A fatal error occurred. Glitch can no longer continue safely and will quit.") + QString("\n\n") + message);
    ::exit(EXIT_FAILURE);
}
...
int main(int argc, char *argv[])
{
    ...
    Q_INIT_RESOURCE(glitch);
    Q_INIT_RESOURCE(glitch_locale);
    ...
    /// 6. Determine availability of data and blocks directory and parse glitch.conf
    ...
    // glitch: links repeatedly have their payment requests routed to this process:
