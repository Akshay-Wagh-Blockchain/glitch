...
const char * const GLITCH_CONF_FILENAME = "glitch.conf";
const char * const GLITCH_PID_FILENAME = "glitchd.pid";
...
public:
    ...
    static inline std::pair<bool,std::string> GetArg(const ArgsManager &am, const std::string& arg)
    {
        ...
        // argument value seen from the command line (so "glitchd -foo=bar
...
{
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\Glitch
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\Glitch
    // Mac: ~/Library/Application Support/Glitch
    // Unix: ~/.glitch
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "Glitch";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // Mac
    return pathRet / "Library/Application Support/Glitch";
#else
    // Unix
    return pathRet / ".glitch";
#endif
#endif
}
...
bool ArgsManager::ReadConfigFiles(std::string& error, bool ignore_invalid_keys)
{
    ...
    const std::string confPath = GetArg("-conf", GLITCH_CONF_FILENAME);
...
fs::path GetPidFile()
{
    return AbsPathForConfigVal(fs::path(gArgs.GetArg("-pid", GLITCH_PID_FILENAME)));
}
...
std::string CopyrightHolders(const std::string& strPrefix)
{
    ...
    if (strprintf(COPYRIGHT_HOLDERS, COPYRIGHT_HOLDERS_SUBSTITUTION).find("Glitch Core") == std::string::npos) {
        strCopyrightHolders += "\n" + strPrefix + "The Glitch Core developers";
    }
