...
#ifndef GLITCH_VALIDATION_H
#define GLITCH_VALIDATION_H
...
#include <glitch/storageresults.h>


extern std::unique_ptr<GlitchState> globalState;
...
using ExtractGlitchTX = std::pair<std::vector<GlitchTransaction>, std::vector<EthTransactionParams>>;
///////////////////////////////////////////
...
static const size_t MAX_CONTRACT_VOUTS = 1000; // glitch
...
void EnforceContractVoutLimit(ByteCodeExecResult& bcer, ByteCodeExecResult& bcerOut, const dev::h256& oldHashGlitchRoot,
    const dev::h256& oldHashStateRoot, const std::vector<GlitchTransaction>& transactions);
...
class GlitchTxConverter{

public:

    GlitchTxConverter(CTransaction tx, CCoinsViewCache* v = NULL, const std::vector<CTransactionRef>* blockTxs = NULL) : txBit(tx), view(v), blockTransactions(blockTxs){}

    bool extractionGlitchTransactions(ExtractGlitchTX& glitchTx);
...
private:
    ...
    GlitchTransaction createEthTX(const EthTransactionParams& etp, const uint32_t nOut);
...
public:
    ...
    ByteCodeExec(const CBlock& _block, std::vector<GlitchTransaction> _txs, const uint64_t _blockGasLimit, CBlockIndex* _pindex) : txs(_txs), block(_block), blockGasLimit(_blockGasLimit), pindex(_pindex) {}
