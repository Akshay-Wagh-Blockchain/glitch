...
#include <glitch/glitchstate.h>

using namespace std;
using namespace dev;
using namespace dev::eth;

GlitchState::GlitchState(u256 const& _accountStartNonce, OverlayDB const& _db, const string& _path, BaseState _bs) :
        State(_accountStartNonce, _db, _bs) {
            dbUTXO = GlitchState::openDB(_path + "/glitchDB", sha3(rlp("")), WithExisting::Trust);
	        stateUTXO = SecureTrieDB<Address, OverlayDB>(&dbUTXO);
}

GlitchState::GlitchState() : dev::eth::State(dev::Invalid256, dev::OverlayDB(), dev::eth::BaseState::PreExisting) {
    dbUTXO = OverlayDB();
    stateUTXO = SecureTrieDB<Address, OverlayDB>(&dbUTXO);
}

ResultExecute GlitchState::execute(EnvInfo const& _envInfo, SealEngineFace const& _sealEngine, GlitchTransaction const& _t, Permanence _p, OnOpFunc const& _onOp){

    assert(_t.getVersion().toRaw() == VersionVM::GetEVMDefault().toRaw());

    addBalance(_t.sender(), _t.value() + (_t.gas() * _t.gasPrice()));
    newAddress = _t.isCreation() ? createGlitchAddress(_t.getHashWith(), _t.getNVout()) : dev::Address();
    ...
    try{
        ...
        if (_p == Permanence::Reverted){
            ...
            glitch::commit(cacheUTXO, stateUTXO, m_cache);
            cacheUTXO.clear();
...
std::unordered_map<dev::Address, Vin> GlitchState::vins() const // temp
{
...
void GlitchState::transferBalance(dev::Address const& _from, dev::Address const& _to, dev::u256 const& _value) {
...
Vin const* GlitchState::vin(dev::Address const& _a) const
{
    return const_cast<GlitchState*>(this)->vin(_a);
}

Vin* GlitchState::vin(dev::Address const& _addr)
{
...
// void GlitchState::commit(CommitBehaviour _commitBehaviour)
// {
...
void GlitchState::kill(dev::Address _addr)
{
...
void GlitchState::addBalance(dev::Address const& _id, dev::u256 const& _amount)
{
...
void GlitchState::deleteAccounts(std::set<dev::Address>& addrs){
...
void GlitchState::updateUTXO(const std::unordered_map<dev::Address, Vin>& vins){
    for(auto& v : vins){
...
void GlitchState::printfErrorLog(const dev::eth::TransactionException er){
    std::stringstream ss;
    ss << er;
