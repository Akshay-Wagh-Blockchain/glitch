...
unique_ptr<CBlockTemplate> BlockAssembler::CreateNewBlock(CAccountViewCache &view, CTransactionDBCache &txCache,
                                          CScriptDBViewCache &scriptCache) {
    // Create new block
    unique_ptr<CBlockTemplate> BlockAssembler:: pblocktemplate(new CBlockTemplate());
...
void static CoinMiner(CWallet *pwallet, int targetHeight) {
    ...
    try {
        ...
        while (true) {
            ...
            unique_ptr<CBlockTemplate> BlockAssembler:: pblocktemplate(CreateNewBlock(accountView, txCache, scriptDB));
            if (!pblocktemplate.get())
