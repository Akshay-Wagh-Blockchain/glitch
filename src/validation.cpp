...
#if defined(NDEBUG)
# error "Glitch cannot be compiled without assertions."
#endif
...
std::unique_ptr<GlitchState> globalState;
...
bool fAddressIndex = false; // glitch
#endif
...
const std::string strMessageMagic = "Glitch Signed Message:\n";
                              ...
                              bool bypass_limits, const CAmount& nAbsurdFee, std::vector<COutPoint>& coins_to_uncache, bool test_accept, bool rawTx)
{
...

    {
        ...
        if(tx.HasCreateOrCall()){
            ...
            uint64_t minGasPrice = glitchDGP.getMinGasPrice(chainActive.Tip()->nHeight + 1);
            uint64_t blockGasLimit = glitchDGP.getBlockGasLimit(chainActive.Tip()->nHeight + 1);
            size_t count = 0;
            for(const CTxOut& o : tx.vout)
                count += o.scriptPubKey.HasOpCreate() || o.scriptPubKey.HasOpCall() ? 1 : 0;
            GlitchTxConverter converter(tx, NULL);
            ExtractGlitchTX resultConverter;
            if(!converter.extractionGlitchTransactions(resultConverter)){
            ...
            std::vector<GlitchTransaction> glitchTransactions = resultConverter.first;
            std::vector<EthTransactionParams> glitchETP = resultConverter.second;

            dev::u256 sumGas = dev::u256(0);
            dev::u256 gasAllTxs = dev::u256(0);
            for(GlitchTransaction glitchTransaction : glitchTransactions){
                sumGas += glitchTransaction.gas() * glitchTransaction.gasPrice();
                ...
                if(txMinGasPrice != 0) {
                    txMinGasPrice = std::min(txMinGasPrice, glitchTransaction.gasPrice());
                } else {
                    txMinGasPrice = glitchTransaction.gasPrice();
                }
                VersionVM v = glitchTransaction.getVersion();
                if(v.format!=0)
                ...
                if(glitchTransaction.gas() < gArgs.GetArg("-minmempoolgaslimit", MEMPOOL_MIN_GAS_LIMIT))
                ...
                if(glitchTransaction.gas() < MINIMUM_GAS_LIMIT && v.rootVM != 0)
                ...
                if(glitchTransaction.gas() > UINT32_MAX)
                ...
                gasAllTxs += glitchTransaction.gas();
                ...
                if(v.rootVM!=0 && (uint64_t)glitchTransaction.gasPrice() < minGasPrice)
            ...
            if(!CheckMinGasPrice(glitchETP, minGasPrice))
                return state.DoS(100, false, REJECT_INVALID, "bad-txns-small-gasprice");

            if(count > glitchTransactions.size())
        ...
        for (CTxMemPool::txiter it : allConflicting)
        {
            LogPrint(BCLog::MEMPOOL, "replacing tx %s with %s for %s GLITCH additional fees, %d delta bytes\n",
...
DisconnectResult CChainState::DisconnectBlock(const CBlock& block, const CBlockIndex* pindex, CCoinsViewCache& view, bool* pfClean)
{
    ...
    globalState->setRoot(uintToh256(pindex->pprev->hashStateRoot)); // glitch
    globalState->setRootUTXO(uintToh256(pindex->pprev->hashUTXORoot)); // glitch
...
std::vector<ResultExecute> CallContract(const dev::Address& addrContract, std::vector<unsigned char> opcode, const dev::Address& sender, uint64_t gasLimit){
    ...
    GlitchTransaction callTransaction(0, 1, dev::u256(gasLimit), addrContract, opcode, dev::u256(0));
    callTransaction.forceSender(senderAddress);
    callTransaction.setVersion(VersionVM::GetEVMDefault());

    
    ByteCodeExec exec(block, std::vector<GlitchTransaction>(1, callTransaction), blockGasLimit, pblockindex);
...
void writeVMlog(const std::vector<ResultExecute>& res, const CTransaction& tx, const CBlock& block){
    boost::filesystem::path glitchDir = GetDataDir() / "vmExecLogs.json";
    std::stringstream ss;
    if(fIsVMlogFile){
        ss << ",";
    } else {
        std::ofstream file(glitchDir.string(), std::ios::out | std::ios::app);
        file << "{\"logs\":[]}";
        file.close();
    }
    ...
    std::ofstream file(glitchDir.string(), std::ios::in | std::ios::out);
...
bool GlitchTxConverter::extractionGlitchTransactions(ExtractGlitchTX& glitchtx){
    std::vector<GlitchTransaction> resultTX;
    ...
    glitchtx = std::make_pair(resultTX, resultETP);
    return true;
}

bool GlitchTxConverter::receiveStack(const CScript& scriptPubKey){
...
bool GlitchTxConverter::parseEthTXParams(EthTransactionParams& params){
    try{
...
GlitchTransaction GlitchTxConverter::createEthTX(const EthTransactionParams& etp, uint32_t nOut){
    GlitchTransaction txEth;
    if (etp.receiveAddress == dev::Address() && opcode != OP_CALL){
        txEth = GlitchTransaction(txBit.vout[nOut].nValue, etp.gasPrice, etp.gasLimit, etp.code, dev::u256(0));
    }
    else{
        txEth = GlitchTransaction(txBit.vout[nOut].nValue, etp.gasPrice, etp.gasLimit, etp.receiveAddress, etp.code, dev::u256(0));
    }
                  ...
                  CCoinsViewCache& view, const CChainParams& chainparams, bool fJustCheck)
{
    ...
    ///////////////////////////////////////////////// // glitch
    GlitchDGP glitchDGP(globalState.get(), fGettingValuesDGP);
    globalSealEngine->setGlitchSchedule(glitchDGP.getGasSchedule(pindex->nHeight + 1));
    uint32_t sizeBlockDGP = glitchDGP.getBlockSize(pindex->nHeight + 1);
    uint64_t minGasPrice = glitchDGP.getMinGasPrice(pindex->nHeight + 1);
    uint64_t blockGasLimit = glitchDGP.getBlockGasLimit(pindex->nHeight + 1);
    ...
    if (block.vtx.empty() || block.vtx.size() > dgpMaxBlockSize || ::GetSerializeSize(block, SER_NETWORK, PROTOCOL_VERSION | SERIALIZE_TRANSACTION_NO_WITNESS) > dgpMaxBlockSize) // glitch
...
///////////////////////////////////////////////////////////////////////////////////////// glitch
        ...
        if(tx.HasCreateOrCall() && !hasOpSpend){
            ...
            GlitchTxConverter convert(tx, &view, &block.vtx);

            ExtractGlitchTX resultConvertGlitchTX;
            if(!convert.extractionGlitchTransactions(resultConvertGlitchTX)){
            ...
            if(!CheckMinGasPrice(resultConvertGlitchTX.second, minGasPrice))
            ...
            dev::u256 gasAllTxs = dev::u256(0);
            ByteCodeExec exec(block, resultConvertGlitchTX.first, blockGasLimit, pindex->pprev);
            ...
            for(GlitchTransaction& qtx : resultConvertGlitchTX.first){
            ...
            {
                for(size_t k = 0; k < resultConvertGlitchTX.first.size(); k ++){
                    ...
                    tri.push_back(TransactionReceiptInfo{block.GetHash(), uint32_t(pindex->nHeight), tx.GetHash(), uint32_t(i), resultConvertGlitchTX.first[k].from(), resultConvertGlitchTX.first[k].to(),
...
bool CChainState::ConnectTip(CValidationState& state, const CChainParams& chainparams, CBlockIndex* pindexNew, const std::shared_ptr<const CBlock>& pblock, ConnectTrace& connectTrace, DisconnectedBlockTransactions &disconnectpool)
{
    ...
    LogPrint(BCLog::BENCH, "  - Load block from disk: %.2fms [%.2fs]\n", (nTime2 - nTime1) * MILLI, nTimeReadFromDisk * MICRO);
    {
        ...
        dev::h256 oldHashStateRoot(globalState->rootHash()); // glitch
        dev::h256 oldHashUTXORoot(globalState->rootHashUTXO()); // glitch
        ...
        if (!rv) {
            ...
            globalState->setRoot(oldHashStateRoot); // glitch
            globalState->setRootUTXO(oldHashUTXORoot); // glitch
...
//////////////////////////////////////////////////////////////////////////
    ...
    for (pindex = chainActive.Tip(); pindex && pindex->pprev; pindex = pindex->pprev) {
        ...
        ///////////////////////////////////////////////////////////////////// // glitch
        uint32_t sizeBlockDGP = glitchDGP.getBlockSize(pindex->nHeight);
        dgpMaxBlockSize = sizeBlockDGP ? sizeBlockDGP : dgpMaxBlockSize;
        updateBlockSizeParams(dgpMaxBlockSize);
        /////////////////////////////////////////////////////////////////////
    ...
    try {
        ...
        while (!blkdat.eof()) {
            ...
            try {
                ...
                // But for Glitch PoS we need to sync this after every block to ensure txdb is populated for
