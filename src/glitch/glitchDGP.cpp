#include <glitch/glitchDGP.h>

void GlitchDGP::initDataEIP158(){
...
bool GlitchDGP::checkLimitSchedule(const std::vector<uint32_t>& defaultData, const std::vector<uint32_t>& checkData){
...
dev::eth::EVMSchedule GlitchDGP::getGasSchedule(unsigned int blockHeight){
    clear();
...
uint64_t GlitchDGP::getUint64FromDGP(unsigned int blockHeight, const dev::Address& contract, std::vector<unsigned char> data){
    uint64_t value = 0;
...
uint32_t GlitchDGP::getBlockSize(unsigned int blockHeight){
    clear();
...
uint64_t GlitchDGP::getMinGasPrice(unsigned int blockHeight){
    clear();
...
uint64_t GlitchDGP::getBlockGasLimit(unsigned int blockHeight){
    clear();
...
bool GlitchDGP::initStorages(const dev::Address& addr, unsigned int blockHeight, std::vector<unsigned char> data){
    initStorageDGP(addr);
...
void GlitchDGP::initStorageDGP(const dev::Address& addr){
    storageDGP = state->storage(addr);
}

void GlitchDGP::initStorageTemplate(const dev::Address& addr){
    storageTemplate = state->storage(addr);
}

void GlitchDGP::initDataTemplate(const dev::Address& addr, std::vector<unsigned char>& data){
    dataTemplate = CallContract(addr, data)[0].execRes.output;
}

void GlitchDGP::createParamsInstance(){
...
dev::Address GlitchDGP::getAddressForBlock(unsigned int blockHeight){
...
void GlitchDGP::parseStorageScheduleContract(std::vector<uint32_t>& uint32Values){
...
void GlitchDGP::parseDataScheduleContract(std::vector<uint32_t>& uint32Values){
...
void GlitchDGP::parseStorageOneUint64(uint64_t& value){
...
void GlitchDGP::parseDataOneUint64(uint64_t& value){
    if(dataTemplate.size() == 32){
        value = dev::toUint64(dev::u256(dev::h256(dataTemplate)));
    }
}

dev::eth::EVMSchedule GlitchDGP::createEVMSchedule(){
    dev::eth::EVMSchedule schedule = dev::eth::EIP158Schedule;
