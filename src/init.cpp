 ...
 * between glitchd, and glitch-qt and non-server tools.
*/
...
void SetupServerArgs()
{
    ...
    gArgs.AddArg("-conf=<file>", strprintf("Specify configuration file. Relative paths will be prefixed by datadir location. (default: %s)", GLITCH_CONF_FILENAME), false, OptionsCategory::OPTIONS);
...
#ifndef WIN32
    gArgs.AddArg("-pid=<file>", strprintf("Specify pid file. Relative paths will be prefixed by a net-specific datadir location. (default: %s)", GLITCH_PID_FILENAME), false, OptionsCategory::OPTIONS);
#else
...
{
    const std::string URL_SOURCE_CODE = "<https://github.com/glitchproject/glitch>";
    const std::string URL_WEBSITE = "<https://glitch.org>";
...
//////////////////////////////////////////////////////////////////////
    ...
    LogPrintf("Using config file %s\n", GetConfigFile(gArgs.GetArg("-conf", GLITCH_CONF_FILENAME)).string());
    LogPrintf("Using at most %i automatic connections (%i file descriptors available)\n", nMaxConnections, nFD);

    // Warn about relative -datadir path.
    if (gArgs.IsArgSet("-datadir") && !fs::path(gArgs.GetArg("-datadir", "")).is_absolute()) {
        LogPrintf("Warning: relative datadir option '%s' specified, which will be interpreted relative to the " /* Continued */
                  "current working directory '%s'. This is fragile, because if glitch is started in the future "
                  "from a different location, it will be unable to locate the current data files. There could "
                  "also be data loss if glitch is started while in a temporary directory.\n",
    ...
    while (!fLoaded && !ShutdownRequested()) {
        ...
        do {
            ...
            try {
                ...
                dev::eth::Ethash::init();
                fs::path glitchStateDir = GetDataDir() / "stateGlitch";
                bool fStatus = fs::exists(glitchStateDir);
                const std::string dirGlitch(glitchStateDir.string());
                const dev::h256 hashDB(dev::sha3(dev::rlp("")));
                dev::eth::BaseState existsGlitchstate = fStatus ? dev::eth::BaseState::PreExisting : dev::eth::BaseState::Empty;
                globalState = std::unique_ptr<GlitchState>(new GlitchState(dev::u256(0), GlitchState::openDB(dirGlitch, hashDB, dev::WithExisting::Trust), dirGlitch, existsGlitchstate));
                dev::eth::ChainParams cp((dev::eth::genesisInfo(dev::eth::Network::glitchMainNetwork)));
                globalSealEngine = std::unique_ptr<dev::eth::SealEngineFace>(cp.createSealEngine());

                pstorageresult.reset(new StorageResults(glitchStateDir.string()));
                if (fReset) {
