...
void checkResult(bool isCreation, std::vector<GlitchTransaction> results, uint256 hash){
...
void runTest(bool isCreation, size_t n, CScript& script1, CScript script2 = CScript()){
    ...
    GlitchTxConverter converter(transaction, NULL);
    ExtractGlitchTX glitchTx;
    BOOST_CHECK(converter.extractionGlitchTransactions(glitchTx));
    std::vector<GlitchTransaction> result = glitchTx.first;
    if(script2 == CScript()){
...
BOOST_FIXTURE_TEST_SUITE(glitchtxconverter_tests, TestingSetup)
